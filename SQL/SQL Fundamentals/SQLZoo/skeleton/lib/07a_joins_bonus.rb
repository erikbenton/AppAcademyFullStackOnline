...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style, (SUM(album_tracks_count.price) / SUM(album_tracks_count.count))
    FROM
      styles
    JOIN (
      SELECT
        albums.*,
        COUNT(tracks.song) as count
      FROM
        albums
      JOIN
        tracks ON tracks.album = albums.asin
      ...
      GROUP BY
        albums.asin
    ) AS album_tracks_count ON album_tracks_count.asin = styles.album
    GROUP BY
      styles.style
    ORDER BY
      (SUM(album_tracks_count.price) / SUM(album_tracks_count.count)) DESC,
      styles.style ASC
    LIMIT
      5
  SQL
